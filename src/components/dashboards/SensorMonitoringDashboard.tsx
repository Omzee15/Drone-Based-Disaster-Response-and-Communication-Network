...
import { SensorReading, DisasterPrediction } from '@/types/sensor';
...
export const SensorMonitoringDashboard: React.FC<SensorMonitoringDashboardProps> = ({
  sensors,
  predictions,
  selectedSensor,
  onSelectSensor,
  ...
  const sensorsByLocation = useMemo(() => {
    const locations: Record<string, SensorReading[]> = {};
  ...
  const getSensorIcon = (sensor: SensorReading) => {
    let color = '';
              ...
              {isLoaded ? (
                ...
                >
                  ...
                  {Object.entries(sensorsByLocation).map(([locationKey, locationSensors]) => {
                    ...
                    const hasCritical = locationSensors.some(s => s.status === 'critical');
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center space-x-2">
                      <div className={`p-1 rounded-full ${
                        sensor.status === 'critical' ? 'bg-red-500/20' : 
                    ...
                    <div className={`text-sm font-medium ${
                      ...
                      {sensor.value.toFixed(sensor.type === 'seismic' ? 2 : 1)} {sensor.unit}
                    </div>
                  </div>
                  ...
                  <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${
                      ...
                      width: `${Math.min(100, (sensor.value / sensor.thresholds.critical) * 100)}%`
                    }}/>
                  </div>
